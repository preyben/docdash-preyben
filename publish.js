...
function generate(type, title, docs, filename, resolveLinks) {
    ...
    var docData = {
        ...
        docs: docs,
        fileType: (docs[0] && docs[0].meta ? docs[0].meta.filename : filename).split('.').pop(),
    };
...
function buildMemberNavModules(items, itemHeading, itemsSeen, linktoFn) {
    var nav = '';
    const trees = {};
    items.forEach(i => {
        ...
        base._methods = find({ kind: 'function', memberof: i.longname });
        base._member = find({ kind: 'member', memberof: i.longname });
    });
    const toSubNav = (tree, key, level = 1, parent = '') => {
        if (!tree) {
            return "";
        }
        const searchKey = parent + '/' + key;
        let nav = `<ul class='menu members' searchKey="${searchKey}" style="margin-left: ${12 * (level - 1)}px;"><div class="collapsed_header" style="font-size: ${24 - (level * 4)}px;padding: 10px 0;">${key}</div>`;
        if (tree._methods) {
            tree._methods.forEach(j => {
                const name = (j.summary || j.description || j.name).replace(/<p>|<\/p>/g, '');
                nav += `<li data-type="method" class="menu" searchKey="${searchKey + '/' + name + '|' + j.name}"  id="${j.memberof.replace('/', '_').replace('module:', '')}-~${j.name}-nav">${linktoFn(j.longname, name).replace(/%2525/g, "%25")}</li>`
            })
            tree._member.forEach(j => {
                const name = (j.summary || j.description || j.name).replace(/<p>|<\/p>/g, '');
                nav += `<li data-type="method" class="menu" searchKey="${searchKey + '/' + name + '|' + j.name}"  id="${j.memberof.replace('/', '_').replace('module:', '')}-~${j.name}-nav">${linktoFn(j.longname, name).replace(/%2525/g, "%25")}</li>`
            })
        } else {
            for (let j in tree) {
                let v = toSubNav(tree[j], j, level + 1, searchKey);
                nav += `<li searchKey="${j}">${v}</li>`;
            }
        }
        return nav + '</ul>';
    }
    for (let i in trees) {
        nav += toSubNav(trees[i], i, 1, '');
    }
    return nav;
}
function buildMemberNav(items, itemHeading, itemsSeen, linktoFn) {
    ...
    if (itemHeading === 'Modules') {
        return buildMemberNavModules(items, itemHeading, itemsSeen, linktoFn);
    }
    var nav = '';
...
function buildNav(members) {
    var nav = '<h2><a href="index.html">' + (env.conf.templates.title || '首页') + '</a></h2>';
    var seen = {};
    var seenTutorials = {};
    var docdash = env && env.conf && env.conf.docdash || {};
    if (docdash.menu) {
